...
Widget refreshingJobsBloc(
    Widget Function(RefreshingJobsFetched pageState) builderFunc,
    ) {
  return BlocBuilder<RefreshingJobsBloc, RefreshingJobsState>(
    buildWhen: (prev, next) {
      return prev != next && next is RefreshingJobsFetched;
    },
    builder: (_, refreshingJobsState) {
      if (refreshingJobsState is RefreshingJobsFetched) {
        return builderFunc.call(refreshingJobsState);
      } else {
        return empty();
      }
    },
