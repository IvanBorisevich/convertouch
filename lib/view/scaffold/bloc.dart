  ...
  return MultiBlocListener(
      listeners: [
        BlocListener<UnitGroupsMenuBloc, UnitGroupsMenuState>(
          ...
          listener: (_, unitGroupSelected) {
            if (unitGroupSelected is UnitGroupSelected) {
              NavigationService.I.navigateBack();
            } else if (unitGroupSelected is UnitGroupsFetched) {
        ...
        BlocListener<UnitsMenuBloc, UnitsMenuState>(
          listenWhen: (prev, next) {
            return prev != next && next.triggeredBy == unitsConversionPageId;
          },
          listener: (_, unitsFetched) {
            if (unitsFetched is UnitsFetched) {
              NavigationService.I.navigateTo(unitsPageId);
...
Widget wrapIntoDialogListenerForUnitGroupsPage(BuildContext context, Widget widget) {
...
Widget wrapIntoDialogListenerForUnitsPage(BuildContext context, Widget widget) {
